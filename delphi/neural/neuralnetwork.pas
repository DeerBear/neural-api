...
// reviewed transformer ../_port_tool.py, which is Pascal-lexer-aware:
// compiler directives inside comments and strings are never touched, so
// the large blocks of commented-out legacy code are preserved verbatim.
// FPC and the AVX family of defines are resolved as undefined; the FPC
// generic-list usage maps to Generics.Collections; compound assignments
// are expanded; the AVX SIMD kernels now live in the neuralsimd unit
// (pure-Pascal by default, optional BASM behind the NEURAL_AVX define).
// The 6 live AVX-any sites were resolved by hand: the rep-movsd fast
// row-copy was dropped in favour of its existing pure-Pascal RTL Move
// twin. Build-config conditionals (Debug / Release / CheckRange) are
// intentionally kept (valid Delphi). Review-verified only -- no Delphi
// toolchain was available in the port environment.
// =====================================================================

interface

uses
  neuralsimd, Classes, SysUtils, Math, SyncObjs, neuralvolume, neuralgeneric,
