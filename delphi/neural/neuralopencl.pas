...
// Hand-converted from ../../neural/neuralopencl.pas via ../_port_tool.py, which
...
uses
  Classes, SysUtils, cl, Winapi.Windows,AnsiStrings,CL_Platform, neuralvolume;
...
const
  ...
  =
    (
    (id: CL_DEVICE_TYPE; Name: 'DEVICE TYPE'),
...
var
  err: integer; // error code returned from api calls
  firstpointer, local_platformids: ppcl_platform_id;
...
begin
  ...
  localCompilerOptions := AnsiStrAlloc(length(FCompilerOptions)+1);
  AnsiStrings.StrPCopy(localCompilerOptions,FCompilerOptions);

  // Build the program executable
  err := clBuildProgram(FProg, 0, nil, localCompilerOptions, nil, nil);

  AnsiStrings.StrDispose(localCompilerOptions);

  if (err <> CL_SUCCESS) then
  begin
    errorlog := @errorlogstr[1];
    loglen := SizeOf(errorlogstr);
    clGetProgramBuildInfo(FProg, FCurrentDevice, CL_PROGRAM_BUILD_LOG, SizeOf(errorlogstr), errorlog, @loglen);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        clGetPlatformInfo(FPlatformIds[i], platform_str_info[k].id, sizeof(buf), @buf, @bufwritten);
      ...
      begin
        ...
        begin
          ...
          begin
            clGetDeviceInfo(local_deviceids[j], device_str_info[k].id, sizeof(buf), @buf, @bufwritten);
            MessageProc(device_str_info[k].Name + ': ' + buf);
          end;

          for k := low(device_word_info) to high(device_word_info) do
          begin
            clGetDeviceInfo(local_deviceids[j], device_word_info[k].id, sizeof(buf), @buf, @bufwritten);
...
var
  ...
  firstpointer, local_deviceids: ppcl_device_id;
  j: integer;
  buf: TNeuralStrBuffer;
  bufwritten: csize_t;
begin
...
begin
  ...
  Result := clEnqueueMapBuffer(FCommands, buffer, blocking, map_flags, {offset=}0, cb,
    {num_events=}0, {events_list=}nil, {event=}nil, @err);
  if (err <> CL_SUCCESS) then
  begin
...
begin
  err := 0;
  localKernelName := AnsiStrAlloc(length(kernelname)+1);
  AnsiStrings.StrPCopy(localKernelName,kernelname);
  ...
  AnsiStrings.StrDispose(localKernelName);
end;
