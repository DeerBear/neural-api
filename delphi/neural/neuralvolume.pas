...
Hand-converted from the FPC/Lazarus source under ../../neural/ via the
reviewed, deterministic recipe in ../_port_tool.py: all {$IFDEF FPC}/
{$IFDEF AVX*} conditionals resolved (FPC & AVX* undefined), the FPC
generic TVolume<T> path replaced by the upstream concrete Delphi branch
(T = TNeuralFloat), fgl -> Generics.Collections, compound assignments
(+= -= *= /=) expanded, and the AVX* SIMD kernels relocated to the
neuralsimd unit (which provides them unconditionally: pure-Pascal by
default, optional BASM behind {$DEFINE NEURAL_AVX}). The few
semantically-subtle {$IFDEF AVXANY} sites were resolved by hand.
Build-config conditionals ({$IFDEF Debug/Release}) are intentionally
kept (valid Delphi). Not compiler-verified (no Delphi toolchain in the
port environment) — review-verified only.
*)
...
// operations. Vectorised SIMD paths live in neuralsimd (enable via the
// NEURAL_AVX project define); the default build is pure Pascal.
...
uses neuralsimd, Classes, SysUtils, Contnrs, Generics.Collections;
...
type
  ...
  public
    ...
    procedure Copy(Original: TVolume; Len: integer);  overload;  {$IFDEF Release} inline; {$ENDIF}
    ...
    procedure SetClass(pClass: integer; value: T);  overload; 
    procedure SetClass(pClass: integer; TrueValue, FalseValue: T);  overload; 
...
begin
  MessageProc := Self.DefaultHideMessages;
end;
